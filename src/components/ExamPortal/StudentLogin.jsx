...
const StudentLogin = ({loading, handleLogin, userId, setUserId, password, setPassword }) => {
    ...
    <div className="container mx-auto flex flex-col items-center justify-center flex-grow px-4">
      <div className="bg-gray-100 rounded-lg shadow-lg p-8 max-w-md w-full">
        ...
        <form autoComplete="off">
          ...
          <button 
            type="submit" 
            className={`w-full py-3 rounded-lg transition duration-200 ${loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-800 hover:bg-blue-900 text-white'}`}
            onClick={handleLogin}
            disabled={loading}
          >
            {loading ? "Wait..." : "Start Exam"}
          </button>
        </form>
